...
| T1.4 | **gemini-service** 8083: move `GeminiService`, `GeminiConfigBuilder`, hotel/flight search, fetch strategies, `ItineraryPlannerFacade`; endpoints `/generate-itinerary`, `/generate-weather`, `/plan`, `/hotels/search`, `/flights/search`, `/reference/*`; keys server-side only; audit rows in own DB | T0.3 | `task/t1.4-gemini-service` | **in-progress** | |
