...
| ID | Task | Depends | Branch | Status | Notes |
|---|---|---|---|---|---|
| T3.1 | CI/CD update: monorepo paths, `docker compose build` + smoke-test job, Cypress e2e against compose (mock auth), keep Aikido SAST | T0.1, T0.2 | `task/t3.1-ci-cd` | **blocked** | **Sole blocker: GitHub Actions is disabled on this repo** — 0 runs ever (the legacy main-era workflows never ran either), the pushed `ci.yml` never registered after two pushes ~5 min apart, 0 check-runs on the pushed commits; enabling needs the repo owner/admin (this PAT gets 403 on `actions/permissions`). **Everything else is done and locally verified.** Shipped `.github/workflows/ci.yml` (5 jobs: ① checks — typecheck ×9 workspaces + shared + api-client smokes; ② `@smart/web` build; ③ compose-smoke — `docker compose build` → `up -d` → poll gateway `:8080/healthz` until `status: "ok"` = all 4 upstreams up, 5-min grace, compose-logs artifact on failure, `down -v` always; ④ cypress — component (self-contained suites) + full e2e **mock mode** against `next build`+`next start`, screenshots-on-failure; ⑤ sast — Aikido config byte-identical to the old workflow) + `.github/dependabot.yml` (npm @ root, docker ×6 service dirs — config only, no bumps); **deleted** production.yaml/preview.yaml with the Vercel-retirement rationale in the ci.yml header (runtime is now compose/ECS; a Vercel deploy would ship a UI pointing at a gateway with no public existence; repo secrets left untouched). Triggers: push → `microservices-develop` + `task/**`, PR → `main`, plus `workflow_dispatch` as the manual escape hatch. **Local proof (all green):** YAML parse (js-yaml), typecheck ×9, both smokes, web build under the exact CI env, e2e **17/17** against the mock production server, component **6/6** via the workflow's exact `--spec` command. Two real findings encoded in the workflow: (1) the component dev server leaves `.next` artifacts that silently break the following production build (server-action 500s + dead hydration → e2e went 6-of-8 red) — workflow wipes `.next` between the suites; (2) web build still needs legacy `NEXT_PUBLIC_SUPABASE_*` env → CI passes placeholder values instead of the legacy repo secrets. **Pre-existing debt proven on a pristine detached `microservices-develop` worktree (identical env → byte-identical failures):** component suites `Itinerary/Profile/Weather.cy.tsx` are monolith-era live-integration specs (real Supabase project + live Gemini key; no stubs, no mock branch) — unrunnable under any CI env, so CI runs the 3 self-contained suites via a workflow-owned `--spec` filter (apps/web untouched); the 3 die at the T3.4 cleanup. **Exactly what remains unproven on real runners:** the compose-smoke job (no Docker in this workspace — its poll script has never executed), the cypress/checks/build-web jobs on a runner, and SAST (never ran anywhere; warning: `fail-on-dependency-scan` + CRITICAL floor may trip on the main-branch legacy lockfile's 3 criticals — a policy call, not a config bug). **Unblock path:** owner enables Actions → push any commit to `task/t3.1-ci-cd` (or Actions tab → "Run workflow") → iterate to a green compose job → flip this row to done with the run URL. |
