...
| ID | Task | Depends | Branch | Status | Notes |
|---|---|---|---|---|---|
| T3.1 | CI/CD update: monorepo paths, `docker compose build` + smoke-test job, Cypress e2e against compose (mock auth), keep Aikido SAST | T0.1, T0.2 | `task/t3.1-ci-cd` | **done** | **First CI run in repo history ALL GREEN: https://github.com/jxtoast/Smart_Itinerary/actions/runs/34042235354** (2026-09-06; repo Actions had been disabled since forever — owner enabled it, trigger push, all 5 jobs success in ~2.5 min: checks, build-web, **compose-smoke** — build → up → gateway `/healthz` `status:"ok"` on a real runner — cypress, sast incl. the Aikido dependency scan passing on this branch's lockfile). Shipped `.github/workflows/ci.yml` (5 jobs: ① checks — typecheck ×9 workspaces + shared + api-client smokes; ② `@smart/web` build; ③ compose-smoke — `docker compose build` → `up -d` → poll `:8080/healthz` until `status:"ok"` (all 4 upstreams), 5-min grace, compose-logs artifact on failure, `down -v` always; ④ cypress — component + full e2e in **mock mode** against `next build`+`next start`, screenshots-on-failure; ⑤ sast — Aikido config byte-identical to the old workflow) + `.github/dependabot.yml` (npm @ root, docker ×6 service dirs — config only, no bumps); **deleted** production.yaml/preview.yaml — Vercel deploy/preview retired with the rationale in the ci.yml header (runtime is now compose/ECS; a Vercel deploy would ship a UI pointing at a gateway with no public existence; repo secrets untouched). Triggers: push → `microservices-develop` + `task/**`, PR → `main`, + `workflow_dispatch`. Everything locally verified before the push (typecheck ×9, smokes, web build under the exact CI env, e2e 17/17, component 6/6). Two findings encoded in the workflow: (1) component dev-server `.next` residue silently breaks the following production build (e2e went 6-of-8 red; hydration died + server-action 500s) — workflow wipes `.next` between the suites; (2) the build still needs legacy `NEXT_PUBLIC_SUPABASE_*` env — CI passes placeholder values, not the legacy repo secrets. Pre-existing debt (proven byte-identical on a pristine detached `microservices-develop` worktree): component suites `Itinerary/Profile/Weather.cy.tsx` are monolith-era live-integration specs (real Supabase + live Gemini key, no stubs/mock branch) — unrunnable in any CI env; CI runs the 3 self-contained suites via a workflow-owned `--spec` filter (apps/web untouched); the 3 die at the T3.4 cleanup. No PR — pushed for lead integration per conventions §4. |
...
| T3.3 | Docs: `docs/ARCHITECTURE.md` — mermaid diagram mirroring the image, diagram↔repo↔AWS mapping table, event catalogue, local runbook, AWS migration runbook | T0.1 | `task/t3.3-architecture-docs` | **done** | Shipped `docs/ARCHITECTURE.md`: box-for-box mermaid of the AWS diagram incl. the CI/CD row (+ the two documented fidelity notes: CodeCommit→GitHub, gateway desired_count=2), a sync save-request sequence (browser → Next `/api/*` rewrite → gateway JWT verify → service → its Postgres) and the async `itinerary.created` → TTL+DLX reminder sequence, the diagram↔repo↔AWS↔compose mapping table, a 4-row event catalogue with broker behaviour notes (ownerEmail fallback, at-least-once/poison, auto-reconnect, fail-soft publish), ports + one-line responsibilities + dev/cognito/mock auth modes, and the AWS env-var swap table (pointers to GETTING_STARTED / LOCAL-VS-AWS / cognito RUNBOOK, never-applied $0 status). Verified honestly: every backticked repo path script-checked with `test -e` (all OK; only intentional `<placeholder>` templates unmatched, their globs resolve), all 3 mermaids render clean via `npx @mermaid-js/mermaid-cli` 11.17.0 (PNG eyeballed), READMEs mined for responsibilities. +1 pointer line each in GETTING_STARTED.md and LOCAL-VS-AWS.md; README.md and all code untouched. |
