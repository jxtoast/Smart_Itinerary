...
| T3.2 | **Terraform AWS scaffold** (NOT applied): vpc-lite, ECR, ECS Fargate ×6 (gateway desired_count=2), RDS ×4 `db.t4g.micro`, S3, Secrets Manager, Cognito, ALB (+optional Route53/WAF), CloudWatch; apply runbook + cost estimate README | T0.3 | `task/t3.2-terraform-aws` | **done** | **`terraform -chdir=infra/terraform validate` → "Success! The configuration is valid."** (Terraform v1.16.0 installed via brew during the task; `fmt -check -recursive` clean; `init -backend=false` success; no cloud credentials configured, **plan/apply never run — no AWS account; validate-only is the acceptance bar**). 9 modules under `infra/terraform/`: network (vpc-lite, public subnets, no NAT — SGs form internet→ALB→gateway→service→DB), ecr (6 repos, compose names), ecs (Fargate cluster; 6 task defs whose env mirrors docker-compose 1:1 with the MinIO→S3 / Mailpit→SES / postgres→RDS / dev-token→Cognito swaps commented inline; gateway `desired_count=2` = the diagram's Instance 1/2; `/healthz` container health checks; Cloud Map namespace = compose's service hostnames; images from ECR), rds (4× db.t4g.micro postgres 16, random passwords → composed DATABASE_URLs), s3 (pdf-exports bucket, locked down), secrets (10 Secrets Manager secrets: GEMINI/AMADEUS keys, generated JWT_DEV_SECRET, AMQP_URL→Amazon MQ, SES SMTP creds, 4× DATABASE_URL — injected as container `secrets`), alb (public ALB → gateway TG, idle_timeout 120s matching the gateway's AI ceiling; WAF/Route53/HTTPS count-gated, default OFF), cloudwatch (6 log groups 7-day retention + SNS + 3 alarm families), cognito. **Cognito: chose the `git mv infra/cognito` → `modules/cognito` option — exactly ONE definition in the repo**; only versions.tf changed (provider *config* removed — modules declare requirements, the root configures) and RUNBOOK path/apply wording updated; root module call is count-gated (`enable_cognito`, default true). README.md: module→diagram table, staged apply runbook (incl. images→ECR push and a `db/init/*.sql`→RDS path via a throwaway Fargate psql task, since RDS has no initdb.d), rough cost table (≈$130–150/mo always-on, Cognito $0 free tier, `destroy`→$0 via force_delete/0-day recovery/skip-final-snapshot), and an explicit "not here" list (Amazon MQ broker, SES verification, web hosting, NAT). Provider pin aws ~>5.0 (resolved 5.100.0; note: `aws_service_discovery_service` needs `namespace_id` INSIDE `dns_config` in this version). Deviations: (1) `infra/cognito/.gitignore` not moved — folded into the root `infra/terraform/.gitignore` (`.terraform*/`, `*.tfstate*`, tfplan, tfvars); `.terraform.lock.hcl` IS committed; (2) out-of-scope stale path comments remain at `apps/web/.env.example` + `services/auth-service/.env.example` (still say `infra/cognito` — nothing outside infra/ touched per scope); (3) RabbitMQ broker itself is deliberately not provisioned (matches the task's module list) — AMQP_URL stays an env swap to a hand-made Amazon MQ broker, documented. |
...
| T3.4 | Cleanup: remove legacy browser service classes, `lib/supabase`, `app/api/*` stubs, secret `NEXT_PUBLIC_*` vars; root `.env.example`; README rewrite | T2.2–T2.5 | | todo | Deferred from T3.2: update the stale `infra/cognito` → `infra/terraform/modules/cognito` path references in `apps/web/.env.example` (2 lines) + `services/auth-service/.env.example` (1 line) — T3.2 git-moved the directory but those files were outside its write scope |
