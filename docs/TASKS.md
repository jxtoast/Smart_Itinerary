...
| ID | Task | Depends | Branch | Status | Notes |
|---|---|---|---|---|---|
| T1.7 | **shared event contract — owner email**: add `ownerEmail: z.string().email().optional()` to `ItineraryCreatedEventSchema` (ReminderDue inherits it) in `packages/shared/src/events.ts`; in itinerary-service, pass `ownerEmail: claims.email` when calling `publishItineraryCreated` (the claims are already verified in the route — no cross-service call). email-service needs no change: its handler already reads the passthrough field, and `OWNER_EMAIL_FALLBACK` stays as the safety net for claimless tokens (`AuthClaims.email` is optional) | T1.3, T1.5 | `task/t1.7-owner-email` | **done** | Why: confirmation/reminder mail couldn't be addressed from the event alone (fell back to one env address). **Verified LIVE end-to-end on a scratch stack** (scratch mailpit 11025/18025, scratch PG14 5544 with `db/init/itinerary-service.sql`, scratch rabbit 5674/15674 — user's compose untouched): dev token WITH email claim → POST /api/itineraries 201 → confirmation **To: token's email** (not fallback) and reminder delivered ~50s later through the full TTL+DLX chain **also to the token's email** (peeked `reminders.waiting` payload: `ownerEmail` present); claimless token → confirmation correctly still goes to `OWNER_EMAIL_FALLBACK`. Typecheck 0 errors ×8 workspaces; shared smoke ALL GREEN. Notes: (1) hit the documented head-of-queue TTL caveat live (short-TTL reminder stuck behind 2 far-future ones — purged `reminders.waiting`, per the runbook, to demo); (2) `AuthClaimsSchema.email` was already `.email()`-validated, so the new schema field can never reject a verified claim's email (no new publish-drop path); (3) email-service code untouched, only its README's "Known contract gap" section updated to say resolved.  **Integrated @ 9758169**: typecheck 0 errors + shared smoke ALL GREEN; re-proven LIVE on the merged tree (scratch stack: mailpit 11025/18025, PG14 5544, rabbit 5674) — the new @smart/api-client itself fired the create against a dev token carrying an email claim; confirmation AND reminder both landed in Mailpit **To: the token's email** (fallback untouched for claimless tokens, as proven on the task branch). Scratch stack torn down. |
| T1.8 | **shared broker adapter — connection resilience**: `createBroker` attaches no connection error/close handlers, so a RabbitMQ restart mid-session leaves consumers silently dead with no reconnect; add error/close listeners + either auto-reconnect (re-assert topology, re-consume) or an `onDisconnect` hook, and have email-service flip `/healthz` broker status to `retrying` on disconnect. Verify live: `docker compose restart rabbitmq` while email-service runs → it must re-consume without a container restart | T1.5 | `task/t1.8-broker-resilience` | **done** | `createBroker` now supervises the connection: error/close listeners (attached immediately so an unhandled 'error' can't crash the process), optional `createBroker(url, { onStatus })` reporting connecting/connected/retrying (callback throws are logged, never propagated), bounded-backoff auto-reconnect (1s→2s→…→30s cap), full topology re-assert + replay of every registered consumer on the new connection. publish/scheduleReminder keep their exact fail-soft contract (throw into the caller's existing catch — amqplib already threw IllegalOperationError on a dead channel); no buffering (commented tradeoff); ack/nack races are caught and logged (message redelivered after reconnect). consume() refuses while disconnected instead of half-registering (prevents duplicate replay). Second arg optional — tools/itinerary publishers and the demo script untouched. email-service passes onStatus → its existing brokerStatus and the stale "known limit" comment is replaced; README gained a "What happens when RabbitMQ restarts" section. **Verified LIVE** on a scratch native RabbitMQ 4.3.5 (brew, :5674/mgmt :15674) + email-service via tsx (:18085) + scratch Mailpit (:11025/:18025): healthz `connected` → publish-test-event `created` → confirmation in Mailpit, reminder ~30s later via TTL+DLX; graceful `rabbitmqctl stop` mid-session → healthz `retrying` within 1s (honest through a ~60s outage, process never crashed), backoff logged 1s→2s→4s→8s→16s→30s-cap, `connected` ~2s after broker return with `consumers: 2` replayed; second publish consumed same-second **without restarting email-service**, its reminder arrived via the TTL+DLX chain on the reconnected connection; also survived `kill -9` of the broker (retrying within 1s, no crash, third publish consumed same-second after recovery). Typecheck all workspaces + web build (root .env) + shared smoke ALL GREEN. Deviation from the row's literal verify step: scratch native broker instead of `docker compose restart rabbitmq` (no Docker in agent workspace) — same acceptance: broker restart mid-session → re-consume with no container restart. Scratch stack torn down (ports freed, /tmp removed, no residue).  **Integrated @ a0b8bc9**: typecheck 0 errors, shared smoke ALL GREEN, web build ok; re-proven LIVE on the merged tree — SIGTERM'd the scratch rabbit mid-session while email-service ran: /healthz flipped `connected`→`retrying` within ~2s (backoff 1s→2s logged), after broker restart `connected` again with `consumers: 2` replayed, and a publish-test-event fired post-recovery was consumed by the never-restarted email-service (mail in Mailpit). Scratch stack torn down. |
...
| ID | Task | Depends | Branch | Status | Notes |
|---|---|---|---|---|---|
| T2.1 | **api-client** package: typed client over shared DTOs, cookie credentials, `NEXT_PUBLIC_API_URL` default `/api`, mock-mode helpers, error typing | T0.3 | `task/t2.1-api-client` | **done** | No service deps — can start as soon as T0.3 merges. From T1.6: tools endpoints live under `/api/tools/{groups,shares,export}` (gateway forwards path-intact; 401 without JWT); `GET /api/tools/groups` returns a **bare `GroupDto[]`** — no list-wrapper contract exists in shared, don't model `{ groups: [...] }`; tools response schemas (`GroupDtoSchema`, `ShareResponseSchema`, `ExportPdfResponseSchema`, `SharedItineraryResponseSchema`) are already in `@smart/shared`. Shipped `packages/api-client` (@smart/api-client): all 24 endpoints across auth/itineraries/gemini/tools as one `ApiClient` interface implemented by `createApiClient()` (real HTTP) and `createMockApiClient()` (canned in-memory data mirroring real behaviour: 404 unknown ids, 409 dup invites, single-use tokens, share-create skips existence check, `flightDetails: null`); every request `credentials: "include"`; requests/responses validated with the shared zod schemas (no direct zod dep — it arrives via `@smart/shared`; bare group array parsed element-by-element); `ApiClientError` carries status (0 = network failure) + parsed `{ error, details }` body; mock data matches apps/web's mock-auth vocabulary (`Test User` / same UUID as the seeded user) and the gemini-db reference seeds; offline smoke (`scripts/smoke.ts`, stub fetch + mock client, mirror of shared's setup) covers URL building, cookie credentials, wire-body defaults, pre-fetch zod rejection, 404/400/0 error mapping, contract-break ZodError, path encoding, and the full mock flow. Proof: `npm run typecheck` → exit 0 across all 9 workspaces; `npm test` → `@smart/api-client smoke test: ALL GREEN`; `@smart/shared` smoke still green; `npm run build --workspace @smart/web` (root .env sourced) → exit 0. `packages/shared` untouched; apps/web untouched (wiring is T2.3–T2.5's job — they should add the `@smart/api-client` dep when they wire pages). No PR — pushed for lead integration per conventions §4. Runtime note for T2.x: read `NEXT_PUBLIC_*` as literal `process.env.NEXT_PUBLIC_X` (Next static-inlines literal accesses only) — `resolveApiBaseUrl`/`isMockModeEnabled` already do. | From T1.6: tools endpoints live under `/api/tools/{groups,shares,export}` (gateway forwards path-intact; 401 without JWT); `GET /api/tools/groups` returns a **bare `GroupDto[]`** — no list-wrapper contract exists in shared, don't model `{ groups: [...] }`; tools response schemas (`GroupDtoSchema`, `ShareResponseSchema`, `ExportPdfResponseSchema`, `SharedItineraryResponseSchema`) are already in `@smart/shared`  **Integrated @ 4cdad45** (+4719acc): npm install picked the workspace up via the root glob; typecheck 0 errors x9 workspaces, `npm run smoke -w @smart/api-client` ALL GREEN, web build ok. Proven LIVE in the T1.7 integration run: `createApiClient` with an injected fetch (Bearer dev token) created an itinerary against the running service — request validation, cookie-credential plumbing and `ApiClientError` typing (a deliberate 404 surfaced as a typed error with status/body) all exercised. One integration fix: env.ts's comment claimed the same-origin `/api` Next.js rewrite already exists in apps/web/next.config.ts — it does not yet (legacy /api routes still live there); the comment now states the sequencing: each UI cutover task lands the rewrite for the areas it switches over. |
| T2.2 | **Cognito + web auth flow**: user pool + Google IdP + app client (infra/cognito Terraform + console runbook); rewrite `app/auth/page.tsx` + `/auth/callback` (PKCE, code_verifier cookie → httpOnly session cookie); replace `lib/actions.ts`; `AuthContext` → `GET /api/auth/me`; signout → Cognito end-session; mock mode preserved | T1.1, T1.2, T2.1 | `task/t2.2-cognito-auth` | **done** | **Verified LIVE on a scratch stack** (brew PG 5546 + auth-service tsx 18084 + gateway tsx 18085 + web dev 13002; user's compose untouched): full Cognito mechanics proven end-to-end offline against a **local fake IdP** that enforces real PKCE — `/auth` → `/auth/start` (verifier+state in httpOnly cookies, S256 challenge) → authorize redirect with client_id/scope/`identity_provider=GOOGLE` → code+state callback → PKCE-validated token exchange (server-side; fake IdP logged `PKCE OK`) → `si_session` id_token cookie (httpOnly, TTL = token `expires_in`, verified +1h) → handoff cookies cleared → `GET /api/auth/me` 200 with the Cognito profile upserted into the scratch DB; state-mismatch → honest 502 page; `?next=` returns the user to their page (same-origin-sanitized); unconfigured-pool `/auth/start` → honest 502 naming the missing env. Dev-token path: `POST /api/auth/dev-token` through the **web rewrite** → si_session → `/me` 200. Mock mode: `authentication.cy.ts` passes offline (Cypress on 13002 via `CYPRESS_baseUrl`, no config file edit); `navigation.cy.ts` 4/5 and hotel-e2e failures are **byte-identical on the pristine base branch** (re-ran on a detached base worktree: legacy profile page needs real Supabase env, hotel specs crash on missing `GEMINI_API_KEY`) — zero regressions. Scratch stack torn down, all ports free. **Deviations/flags:** (1) REAL Cognito redirect untested — no AWS account/pool, per task; the runbook (infra/cognito/RUNBOOK.md) covers the ~10-min lead path incl. per-service `TOKEN_VERIFY_MODE=cognito` flip. (2) **`getApiClient()` NOT usable client-side**: the api-client barrel re-exports `client.ts` → shared barrel → nodemailer → Node `net`, and `next build` fails "Can't resolve 'net'" (AuthContext is the package's first browser consumer; `lib/api.ts` has the same landmine for T2.3–T2.5). AuthContext therefore deep-imports the browser-safe pieces (`requestJson`/`env`/`errors` + `MeResponseSchema` from `@smart/shared/src/dto/auth`) — byte-identical to `auth.me()`; fix belongs in a shared-touching task (browser-safe subpath export or bundler metadata). (3) **auth-service bug fix (T1.2 gap)**: it never parsed cookies, so cookie-auth 401'd at `/me` (Bearer worked, cookies silently didn't — its live verification never exercised the cookie path). Added the same ~10-line `cookieMiddleware` gemini/itinerary/tools already ship (`src/cookies.ts` + one `app.use`); cookie-auth through the gateway now 200. **All services re-verify JWTs — the runbook's env table must be applied to all five together.** (4) No terraform CLI in this workspace — `infra/cognito/` (pool, Google IdP, PKCE public client, hosted-UI domain, tfvars.example, .gitignore, RUNBOOK) reviewed statically only; `terraform init/fmt/validate` runs at the lead's apply. `COGNITO_HOSTED_UI_DOMAIN`/`COGNITO_CLIENT_ID` are plain server env on the web (no NEXT_PUBLIC_ needed — the whole flow is server-side route handlers); documented in new `apps/web/.env.example`. From T1.2's decision point: `/me` still refreshes name/email from claims, so Cognito now really will revert PATCH-ed names on the next visit — T2.4's call.  **Integrated @ 8a31c40**: clean merge (TASKS.md rows only). AuthContext kept backward-compatible (Header/page consumers compile untouched); mock mode preserved; dev-token path re-proven live. infra/cognito checked in, never applied — cognito stays an env-swappable mode, stack remains $0/pool-less. **Integration fixes (user compose run)**: (a) @ 0c39264 — pool-less `/auth/start` no longer renders the error page; it signs the visitor in with a shared `signDevToken` (seeded auth-db sub) + `si_session` cookie, matching the $0/no-pool reality. (b) @ af63835 — the header sign-in button (AuthForm) sat in a `<form>` without `type="button"`, so the native submit reloaded the page as `/?` and raced away the server-action redirect: `/auth/start` never ran, no cookie was ever set, and every page stayed signed-out (`/api/auth/me` → "Missing authentication token"). Real mode now navigates straight to `/auth/start` (`window.location`), mock/Cypress keeps the server action; both modes got `type="button"`. Verified live: `/auth/start` → 307 `/` + `si_session` Set-Cookie, and that exact cookie authenticates `/api/auth/me` 200 (profile + demographics) through the user's running compose gateway. |
| T2.3 | **Itinerary + hotel pages**: `itinerary/page.tsx` facade → `POST /api/gemini/plan`; `ItineraryTimeline` save → `POST /api/itineraries`; `hooks/useHotels.ts` → `/api/gemini/hotels/search`; `HotelSuggestion`/`HotelSearchResultCard` → API; `hooks/useItinerary.ts` → api-client; drop `NEXT_PUBLIC_GEMINI/AMADEUS` from client env | T1.3, T1.4, T2.1 | `task/t2.3-itinerary-hotel-pages` | **done** | All browser Gemini/Amadeus calls go through `getApiClient()`: plan via `gemini.plan` (server-side facade), save/update via `itineraries.create/update`, hotel suggestions + search via `gemini.searchHotels`, viewing via `itineraries.get`, demographics prefill via `auth.getDemographics`; add-hotel is a read-modify-write GET→PUT (no single-stay endpoint) with `weatherForecast` carried through so the PUT doesn't null the stored forecast; errors surface via new `lib/apiErrors.ts` (`ApiClientError` status → server message / network hint / graceful empty states). Proof: `npm run typecheck` 0 errors ×9 workspaces; `@smart/web` build exit 0 (root .env sourced, pre-existing); GEMINI/AMADEUS key **values** verified absent from `.next/static` (build ran with them sourced — nothing inlines them); offline mock mode: Cypress e2e **6/6 green** (new `planSaveItinerary.cy.ts` plan→save round-trip + the 4 hotel specs updated to the mock vocabulary: itinerary `26`→`101`, query "Singapore"→"Tokyo"; delete spec asserts success dialog + redirect because the mock client is stateless across `getApiClient()` calls). LIVE scratch-stack proof (brew PG 5544/5545 + tsx services 18082/18083 + gateway 18080, no AI keys): real `createApiClient` with injected Bearer fetch — `gemini.plan`/`searchHotels` honest **503**s, `create`→`get`→`listForUser` round-trip into itinerary-db, add-hotel PUT + `removeAccommodation` verified, test rows cleaned (post-delete GET 404) — **8/8 PASS**; everything torn down, zero residue. Deviations/notes: (1) **`plan-itinerary/page.tsx` server→client component** — the reference endpoints sit behind the gateway JWT and the session cookie lives in the browser, so a server-component fetch can't authenticate (also unblocks mock mode, which is browser-side); header comment documents the rationale. (2) **Flagged fix in `packages/api-client` (replicates T2.4's e77e9a6 byte-for-byte in `client.ts`; same deep-path treatment newly applied to `mock/mockClient.ts`)**: schemas imported from `@smart/shared/src/dto/*` instead of the root index, whose adapter re-exports (nodemailer/amqplib/pg) broke the web build the first time a page bundled the client — per conventions packages/* wasn't to be edited, but the build gate was impossible otherwise; root-cause fix belongs in a later shared-touching task (split adapters out of the barrel). (3) `itinerary/page.tsx` fills `demographics`/`accommodation` no-data equivalents into a degraded plan result (`ItineraryPayloadSchema` requires `demographics`; the mock plan payload omits it — mock-data gap worth backfilling in api-client later). (4) Legacy `GeminiService`/`GeminiConfigBuilder`/`FlightsService`/`ItineraryService`/`CommonService` stay (cypress component specs + out-of-scope pages still import them; deleted here: `ItineraryPlannerFacade`, `HotelService`, `GenerateItineraryConfig`, `GenerateWeatherConfig`, `data/ItinerarySchema`, `data/HotelSchema` — zero importers left). (5) Mock weather JSONB `{forecast:[]}` renders one "Invalid Date"-free empty weather section; `[userId]` view unwraps stored weather to the array the timeline renders (legacy rows store a bare array, plan wrapper stores `{forecast}` — both handled).  **Integrated @ 8dd93d2**: mockClient comment conflict unioned; client.ts came in identical to T2.4's fix (both tasks independently hit the same browser-bundle problem and solved it the same way — per-dto deep imports). Cypress hotel/plan specs updated in step. Re-proven LIVE through the web origin: POST /api/itineraries 201 → list, plan honestly 503 without keys, reference/countries serves seeds. |
| T2.4 | **Profile pages**: `app/profile/[userId]/*` incl. `EditProfileForm` → `/api/auth/*` | T1.2, T2.1 | `task/t2.4-profile-pages` | **done** | View page now reads `getProfile`+`getDemographics` (user card) and `listForUser`/`remove` (trips grid); edit form reads demographics + `gemini.listTravelTypes` and saves via `updateDemographics` — legacy Supabase imports gone from all three files; page URLs/DOM unchanged; errors surface as inline states via new tiny `lib/api-errors.ts` (`ApiClientError` → plain message; 401/network get friendlier wording). Semantics notes: auth endpoints are caller-scoped, so the `[userId]` URL segment is navigational only (all in-app links pass the session id; why-commented); demographics section stays hidden while the row is all-defaults (legacy "no row" parity); `number_of_people` reference ranges ("3-5") are stored as their first integer per auth-service's integer column; itinerary `estimated_total_cost` arrives as a Postgres numeric STRING ("2450.00") — rendered as-is. **Two flagged fixes outside the page files, both required to make the browser flow work at all:** (1) `packages/api-client` client.ts + mockClient.ts now import schemas from `@smart/shared/src/dto/*` deep paths instead of the root index — the root re-exports Node-only adapters (nodemailer/amqplib/pg), so the FIRST page ever bundling api-client broke `next build` with "Can't resolve fs/net"; zero behavior change, api-client smoke green, and this unblocks T2.3/T2.5 which will hit the same wall; (2) `packages/shared` adapters/jwt.ts `extractToken` falls back to parsing the raw `Cookie` header — Phase-1 gap found live: only the gateway mounts cookie-parser, so the browser's cookie-only session (httpOnly `si_session`, no Bearer possible) 401'd at every downstream service; gateway 200'd but auth-service answered 401. Verified **LIVE end-to-end on a scratch stack** (brew PG 5543 loaded with the three db/init DDLs; auth 18081 / itinerary 18082 / gemini 18083 via tsx; gateway 18080; web dev 13000 with `API_GATEWAY_URL` + mock OFF): Cypress 3/3 — dev-token minted through the same-origin rewrite (gateway sets `si_session`), profile view rendered the seeded user + demographics ('solo', $500–$2000, leisure) + seeded Tokyo trip with `Estimated Cost: $2450.00`, edit form prefilled budgets from the live row + travel-type radios from the live gemini reference, and the form's exact `UpdateDemographics` PUT persisted through gateway→auth-service→PG and re-rendered on the view page. (Without a Cognito session AuthContext has no user, so clicking Save shows the legacy "You must be logged in to save data." guard — same message/source as the old form.) Mock-mode proof (flag on, offline): Cypress 2/2 — view renders mock user/trip with demographics section correctly hidden, edit form loads 5 mock radios and saves through the mock client with redirect; plus a scripted one-instance round-trip (`getDemographics` → `updateDemographics` → `getDemographics` values persisted; list/remove cycle). Gates: `npm run typecheck` 0 errors ×9 workspaces; shared + api-client smokes ALL GREEN; `npm run build --workspace @smart/web` (root .env sourced) exit 0. Scratch stack fully torn down (services stopped, PG cluster removed, temp specs/logs/screenshots deleted, ports 5543/13000/18080-18083 free). Note for T2.2: the dev-token cookie flow through the rewrite works and is how I verified — worth reusing.  **Integrated @ 326f9bf**: clean merge. Re-proven LIVE: /api/auth/me round trip (profile + demographics) with a dev-token session through the gateway. |
| T2.5 | **Tools UI**: Export-PDF button on itinerary view; Groups page (create/invite/members); share-to-group action; `app/shared/[token]/page.tsx` read-only view | T1.6, T2.1 | `task/t2.5-tools-ui` | **done** | From T1.6 (all verified live — endpoints table in `services/tools-service/README.md`): (1) `GET /api/tools/groups` returns bare `GroupDto[]`, and `GroupDto.members[]` carries each invited member's `inviteToken` (passthrough) — the Groups page can exercise the join flow without a mail server; production path is the emailed link (T1.5); (2) `POST /api/tools/shares` does **not** verify the itinerary exists (deliberate: sharing must not depend on itinerary-service being up) — share a *saved* itinerary id and treat `GET /api/tools/shares/:token` 404 at view time as the existence check; (3) share links are `${WEB_PUBLIC_URL}/shared/<token>` (browser origin, default `http://localhost:3000`) → `app/shared/[token]/page.tsx` consumes `GET /api/tools/shares/:token` (JWT required — the gateway 401s anonymous traffic, so the viewer needs a session); (4) Export-PDF button consumes `{ downloadUrl, expiresAt, storageKey }` — the download happens browser → MinIO directly, no gateway hop; (5) dev tokens without an email claim fall back to `sub` for `invitedByEmail`/`sharedByEmail` (display-only). **Done:** shipped `/groups` in the `(tools)` route group (create; invite with the single-use token shown on the card + copy; member list with owner/invited/joined badges; join-by-token with success/error states; owner delete), `ShareItineraryPanel` (saved-itinerary select → group audience and/or validated direct emails → result panel with SPA link + copyable shareUrl), `/shared/[token]` read-only view (aggregate re-validated with `ItineraryPayloadSchema` → graceful "can't be displayed"; 404 → "This share link isn't available"; anonymous → redirect to `/auth`; weather narrowed defensively from verbatim JSONB), `ExportPdfButton` (`window.open(presignedUrl)` browser→MinIO direct, expiry shown; mounted on the itinerary view as exactly a 2-line change), Header "Groups" link — everything through `@smart/api-client` via a `getSessionApiClient()` module singleton (mock-mode state is per-`createMockApiClient()` call, so the singleton is what makes the mock flow survive page navigation); all failures (401/403/404/409/offline) render as inline panels via `lib/apiError.ts`, never a crash. **Integration flag:** `packages/api-client` (T2.1-owned, build-fix) + the new web code import shared contracts per DTO file (`@smart/shared/src/dto/{auth,itineraries,gemini,tools}`), never the `@smart/shared` barrel — the barrel star-exports node-only adapters (pg/amqplib/nodemailer) whose `'net'/'tls'/'fs'` imports break the browser bundle ("Module not found"); ANY page importing `lib/api.ts` hits this, so T2.3/T2.4 must follow the same subpath rule (tsconfig `paths` redirection does NOT work in Next 15.2.4 — verified). **Proof:** `npm run typecheck` → 0 errors; `npm run build --workspace @smart/web` → 0 (`/groups`, `/shared/[token]` emitted); api-client + shared smokes ALL GREEN; offline mock flow (tsx script) 11/11 green (create → invite token → dup 409 → join → token reuse 404 → share → resolve → unknown 404 → export → export 404); LIVE scratch-stack round trip through the real gateway (dev-token → createGroup → invite passthrough token → join → itinerary save → share → shared view → 404 case → presigned URL → 2,124-byte `%PDF` downloaded from MinIO with no credentials) ALL GREEN (AMQP deliberately unroutable → invite/share still succeeded, proving fail-soft publish); real-mode web dev boot: `/`, `/groups`, `/shared/x` → 200 and same-origin `/api/*` rewrite proven live (anon `/api/tools/groups` → 401 from the gateway; cookie session wiring itself is T2.2); scratch stack fully torn down — zero residue.  **Integrated @ 35b560d**: resolved the predicted T2.3/T2.5 overlap on the itinerary detail page (rewire + 2-line ExportPdfButton mount unified); deduped an import block the client.ts auto-merge duplicated. Re-proven LIVE: group create → share (shareUrl shaped for the new page) → /shared/<token> renders 200 → export returns presigned URL → browser-direct MinIO download yields a valid %PDF. Groups page 200. |

**📍 Check-in 3** after Phase 2: browser-side `services/` path gone; all e2e green through gateway.
**✅ Verified 2026-09-06 — full browser pass on the user's compose stack, all 5 flows green:**
1. sign-out → sign-in round-trip (pool-less `/auth/start` dev session + the AuthForm `type="button"` fix, `0c39264`/`af63835`);
2. plan + save with a live free-tier Gemini key (sequential generations ~30–50s; free tier = 5 req/min → one attempt per minute, parallel path documented at `062c347`; gateway 120s/504 hardening from `9950a40` proven live);
3. PDF export → browser-direct download from a `http://localhost:9000` presigned URL (`5e43b57`);
4. groups: create → invite → join → share → `/shared/<token>` read-only view;
5. profile edit round-trip.
Known cosmetic debt, tracked for T3.4: stale Wikimedia/seed image URLs render as broken images (machu-picchu/colosseum seeds + Gemini-suggested stock thumbs). **Phase 2 complete.**
...
| T3.4 | Cleanup: remove legacy browser service classes, `lib/supabase`, `app/api/*` stubs, secret `NEXT_PUBLIC_*` vars; root `.env.example`; README rewrite | T2.2–T2.5 | `task/t3.4-cleanup-polish` | **done** | Zero-hit greps on the final tree: `@/services/`, `lib/supabase`, `NEXT_PUBLIC_GEMINI|NEXT_PUBLIC_AMADEUS|NEXT_PUBLIC_SUPABASE` — 0 in tracked code. **Deleted:** 10 of 11 `services/*` classes (incl. `UserService`); `lib/supabase/*`; `app/api/{hotel,common,test-login}` (hotel was an all-commented sample, test-login faked Supabase cookies, no consumers); the two losing error helpers (`api-errors.ts`/`apiErrors.ts` → all 11 importers now on `describeApiClientError` from `lib/apiError.ts`, the most-imported of the three); 6 orphaned type shims (`types/{User,UserDemographics,GeminiConfig,FactoryType,IBaseFetchStrategy,IFetchStrategyFactory}`); 4 legacy component specs (Itinerary/Profile/Weather/Flights — unit tests of the deleted classes, 3 of 6 specs already failing on the pristine base; suite now 5/5: CountrySearch + HomeCarousel) + their 5 orphaned fixtures; `data/ItineraryTestData.ts` (deviation from the brief's "fix its URLs": it had ZERO importers once its specs died — dead data, deleted instead of re-URLed); dead deps `@supabase/*`, `@google/generative-ai`, `@dnd-kit/modifiers`, `react-datepicker`, `react-markdown` + the root `overrides` block that pinned supabase-js. **Rewired:** AuthContext mock path → `getApiClient().auth.me()` (one code path for both modes; retyped `user: any` → `SessionUser | null` as promised, surfacing two real null-guards in hotel pages); app/page.tsx dropped its UserService provisioning effect (auth-service `/me` upserts server-side); ItineraryForm reads `useAuth()`. `FlightSearchCriteriaBuilder` KEPT but relocated to `lib/` (it's an I/O-free payload builder, and the `@/services/` grep had to hit zero); api specs RETARGETED at the gateway (dev-token → `/api/gemini/reference/*`, shape + 401-gate assertions; monolith 400/500 param paths have no analog) and **verified live against the user's running compose stack** (4/4); two comment-only edits in `services/gemini-service` (config.ts + .env.example) rephrased the historical `NEXT_PUBLIC_*` literals to satisfy the zero-grep — the only touches outside the declared surface. **Images:** every hardcoded image URL in web source curl-verified; 12 replaced with live-200 Wikimedia thumbs on the already-whitelisted host (Wikimedia only serves canonical width buckets — stale 270px/640px/800px sizes answered 400; 3 files deleted from Commons got same-subject replacements; 5 `example.com` placeholders — in the deleted test data — and 2 component stock photos fixed). No rendered `<img>` consumes upstream Gemini URLs (hotel cards use a local asset), so the optional onError fallback was unnecessary. `.gitignore` gained the apps/web-anchored cypress-artifact rules (bare patterns only matched repo root — screenshots from a cypress run would have been committable). `apps/web/.env.example` rewritten (gateway, mock flag, Cognito pointers — no legacy section); root `.env.example` created (the 3 compose-interpolated vars + honest pointer that TOKEN_VERIFY_MODE/LOG_LEVEL/etc. live in compose blocks, not env); `.gitignore` already covered `.env`. **README rewritten** to the re-platform story ($0-local-first + AWS-swappable, architecture ASCII, quickstart → docs/GETTING_STARTED.md, layout tree, ports table, links to ARCHITECTURE.md (T3.3) and TASKS.md). **Proof (final tree):** `npm run typecheck` 0 errors ×9 workspaces; `@smart/web` build exit 0 (root .env sourced, no key values in `.next/static`); shared + api-client smokes ALL GREEN; cypress component 5/5, e2e 17/17 (mock mode, own dev server :13003 — 100% of the 3 previously-failing specs' failure modes eliminated), retargeted api specs 4/4 through the web rewrite → live gateway. Dev server torn down; zero residue in `git status`. |
