...
| T1.5 | **email-service** 8085: consumers for `itinerary.created` / `itinerary.shared` / `group.invited`; reminders via RabbitMQ TTL+DLX (`reminders.waiting` → `reminders.due`); SMTP mailer; HTML templates | T0.3 | `task/t1.5-email-service` | **in-progress** | |
